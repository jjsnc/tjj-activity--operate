...
<template>
	<div :class="['pullUpBox', pulldown ? 'hasPulldown' : '']" id="pullUpBox" :style="pullDownStyle">
		...
		<slot></slot>
        <div v-show="isLoadMore && enoughHeight" class="moreText">{{moreText}}</div>
	</div>
</template>
...
export default {
	...
	watch: {
		data() {
            ...
            this.$nextTick(() => {
                console.log(document.getElementById('pullUpBox').offsetHeight, window.innerHeight)
                this.enoughHeight = document.getElementById('pullUpBox').offsetHeight > window.innerHeight;
                console.log(this.enoughHeight)
            })
        },
        noData(){
