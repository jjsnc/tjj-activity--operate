<template>
  <div class="container">
    ...
    <div class="recharge_top">
      ...
      <!-- 输入手机号提醒 -->
      <p class="tips" :style="{color:(tips_state==true?'#FF430A':'#333')}">{{tips}}</p>
      ...
      <div class="price_lists">
        <div
          class="price_list"
          v-for="(item,key) in price_list.slice(0,6)"
          :key="key"
      ...
      <!-- 充值油卡的地方  -->
      <div class="iol">
        <p class="iol_title">充值加油卡</p>
        <p class="time_tips">*每日22:45至次日凌晨2:00为油卡系统维护时间，可能会延迟到账，请耐心等候。</p>
        <img :src="bannerImg" alt srcset />
      </div>
    ...
    <pay  :pay_show="pay_states" @pay="pay_platform"> </pay>
  </div>
</template>
<script>
let vm = null;
...
export default {
  ...
  methods: {
    ...
    checkTime(){
      ...
      //0点时间 
        ...
        //今日22.45
       let startTime =  zero+86400000-4500000;
       // let startTime =  zero+86400000-2100000-7110000;
        let entTime =  zero+86400000+7200000;

        console.log(startTime);
        console.log(entTime);
        console.log(nowTime);
         console.log(zero);
        if(startTime<=nowTime&&nowTime<=entTime){
          alert("zai")
        }else{
          alert("buzai")
        }
    ...
    getInfo() {
      ...
      this.$http.get(URL.getRechargeList, { params }).then(res => {
        ...
        res = res.data;
        if (res.result == 1) {
          this.bannerImg = res.imageUrl
            ? res.imageUrl
            : require("../../assets/images/bannerImg.jpg");
          ...
          if (Array.isArray(res.data) && res.data.length > 1) {
            ...
            this.$.errorLogUpload(
              ...
              params,
              "",
              req.request.responseURL,
              res.data,
              req.config.method,
              "未输入手机号码数据为空" + res.message,
              req.status
            );
            //话费
          }
    ...
    getInfoUser() {
      ...
      //
      this.$http
        .get(URL.getRechargeList + "/showLoading/1", { params })
        .then(res => {
          console.log(res.data);
          var req = res;
          res = res.data;
    ...
    //                 this.phone_num +
    //                 "</span>充值" +
    //                 checkData.data.info.denomination +
    //                 "元吗?话费充值立即生效，无法取消",
    //               btns: [
    //                 {
    //                   title: "取消",
    ...
    //                             params,
    //                             "",
    //                             result_q.request.responseURL,
    //                             result,
    //                             result_q.config.method,
    //                             "创建订单result不等于1",
    //                             result_q.status
    //                           );
    ...
    //               show: true,
    //               message:
    //                 "已超出每月可享受补贴次数～请确认是否以优惠价<span style='color:#ff430a;font-weight:600' ' >(" +
    //                 checkData.data.info.discount +
    //                 "元)</span>充值",
    //               btns: [
    //                 {
    //                   title: "取消",
  ...
  mounted() {
    ...
    isLogin(
      "131520893756329851",
      "d369c6136cc12b64",
      "ac1b0669-1536-4be7-b761-f15f04a042f6"
    );
    //  this.$.checkLogin();
    // if (xcx_user) {
    ...
    window.filterInput = (val, old) => {
      ...
      var num = val.replace(/\s*/g, "").replace(/(^\+?86)/g, "");
      num = num.replace(/[^0-9]/, "");
      ...
      if (3 <= num.length && num.length < 7) {
        if (this.add) {
          ...
          let _toIndex =
            this.inputIndex == 4 ? this.inputIndex + 1 : this.inputIndex;
  ...
  watch: {
    phone_num(val, old) {
      ...
      // this.phone_num = /(^\+?86)/.test(this.phone_num) ? this.phone_num.replace(/(^\+?86)/g,'') : this.num;
      var val = val.replace(/\s*/g, "").replace(/(^\+?86)/g, "");
      val = val.replace(/[^0-9]/, "");
...
.android .price_list {
  border: 0.12px solid rgba(205, 205, 205, 1);
}
